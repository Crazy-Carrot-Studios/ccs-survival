...
namespace CCS.Modules.CharacterController.Netcode.Editor
{
    public static class CCS_NetcodeRegistryAuditUtility
    {
        ...
        public static void RunFullAudit()
        {
            ...
            for (int i = 0; i < CCS_NetcodeConstants.RequiredNetworkPrefabPaths.Length; i++)
            {
                AuditPrefabAsset(
                    CCS_NetcodeConstants.RequiredNetworkPrefabPaths[i],
                    registeredPaths);
            }
        ...
        public static bool ValidateAuditRules(List<string> failures)
        {
            ...
            for (int i = 0; i < CCS_NetcodeConstants.RequiredNetworkPrefabPaths.Length; i++)
            {
                string path = CCS_NetcodeConstants.RequiredNetworkPrefabPaths[i];
                ...
                {
                    failures.Add($"{path} is not registered in {CCS_NetcodeConstants.TestNetworkPrefabsListPath}.");
                    passed = false;
                }
            }
        ...
        private static void AuditHashTargets()
        {
            ...
            for (int i = 0; i < targetHashes.Length; i++)
            {
                ...
                for (int pathIndex = 0; pathIndex < CCS_NetcodeConstants.RequiredNetworkPrefabPaths.Length; pathIndex++)
                {
                    GameObject prefab = AssetDatabase.LoadAssetAtPath<GameObject>(
                        CCS_NetcodeConstants.RequiredNetworkPrefabPaths[pathIndex]);
                        ...
                        && CCS_NetcodeNetworkObjectHashUtility.GetHash(networkObject) == targetHash)
                    {
                        Debug.Log(
                            ...
                            + $"at {CCS_NetcodeConstants.RequiredNetworkPrefabPaths[pathIndex]}");
                        mapped = true;
                    }
                }
                    ...
                    for (int sceneIndex = 0; sceneIndex < sceneObjects.Length; sceneIndex++)
                    {
                            ...
                            && CCS_NetcodeNetworkObjectHashUtility.GetHash(sceneObject) == targetHash)
                        {
                            Debug.Log(
                                ...
                                + $"in {CCS_NetcodeConstants.MasterTestScenePath}");
                            mapped = true;
                        }
                    }
