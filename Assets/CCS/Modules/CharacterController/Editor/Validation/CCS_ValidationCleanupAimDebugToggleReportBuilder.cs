...
namespace CCS.Modules.CharacterController.Editor
{
    public static class CCS_ValidationCleanupAimDebugToggleReportBuilder
    {
        public static string WriteReport()
        {
            ...
            builder.AppendLine("- Bool: forceRevolverAimSetupPose (Inspector: Force Revolver Aim Setup Pose)");
            builder.AppendLine("- Default: false");
            builder.AppendLine("- Interface: CCS_IRevolverAimSetupPoseDebugSource");
            builder.AppendLine();
            builder.AppendLine("## Aim animator connection");
            builder.AppendLine("- Script: `" + AimAnimatorSourcePath + "`");
            builder.AppendLine("- Strategy: desired presentation = gameplay IsAiming OR diagnostics ForceRevolverAimSetupPose");
            builder.AppendLine("- Debug source resolved once via serialized reference or CCS_RevolverAimSetupPoseDebugRegistry");
            builder.AppendLine();
            builder.AppendLine("## Right-hand visual revolver attachment");
            builder.AppendLine("- Driver: CCS_PlayerEquipmentVisualController.SetDiagnosticsRevolverAimSetupPoseActive");
            builder.AppendLine("- Visual-only preview uses right-hand socket + equipped fit profile when gameplay ownership is absent");
            builder.AppendLine("- Preview removed/restored when debug bool turns off");
            builder.AppendLine();
            builder.AppendLine("## Production player prefab cleanup");
            builder.AppendLine("- CapsuleVisual removed from PF_CCS_CharacterController_Player_Networked");
            builder.AppendLine("- VisualGlasses removed from PF_CCS_CharacterController_Player_Networked");
            builder.AppendLine();
            ...
            builder.AppendLine("- Force Revolver Aim Setup Pose is presentation-only.");
            builder.AppendLine("- CapsuleVisual and VisualGlasses removed from production player prefab.");
            ...
            builder.AppendLine("- Force Revolver Aim Setup Pose available on validation scene only.");
