...
// =============================================================================
// SCRIPT: CCS_NetworkPlayerController
...
{
    [MovedFrom(true, "CCS.Modules.CharacterController.Tests.Netcode", "CCS.Modules.CharacterController.Tests.Netcode.Runtime", "CCS_ControllerTestNetworkPlayerBehaviour")]
    [DefaultExecutionOrder(-200)]
    public sealed class CCS_NetworkPlayerController : NetworkBehaviour
    {
        #region Variables
        ...
        {
            CCS_NetworkPlayerController[] behaviours =
                Object.FindObjectsByType<CCS_NetworkPlayerController>(
                    FindObjectsInactive.Include, FindObjectsSortMode.None);
            for (int i = 0; i < behaviours.Length; i++)
            {
                CCS_NetworkPlayerController behaviour = behaviours[i];
                if (behaviour != null)
                {
        ...
        private void ResolveReferences()
        {
            ...
            {
                Transform visual = transform.Find(CCS_NetcodeConstants.CapsuleVisualName);
                bodyRenderer = visual != null ? visual.GetComponent<Renderer>() : GetComponentInChildren<Renderer>();
            }

            glassesTransform = transform.Find(CCS_NetcodeConstants.GlassesVisualName);
            ...
            {
                Transform nameplateRoot = transform.Find(CCS_NetcodeConstants.NameplateRootObjectName);
        ...
        {
            bool gameplaySceneActive = CCS_MultiplayerSpawnUtility.IsMasterTestSceneActive();
        ...
        private void EnsureCameraFollowAnchor()
        {
            ...
            {
                CCS_PlayerDisplayProfile profile = ResolveDisplayProfile();
                if (profile != null)
                {
        ...
        private void ApplyDisplayProfileLayout()
        {
            ...
            CCS_PlayerDisplayProfileApplicator.ApplyVisualLayout(gameObject, profile);
            if (IsOwner)
            {
                CCS_PlayerDisplayProfileApplicator.ApplyGameplayProfiles(gameObject, profile);
            }
        }

        private CCS_PlayerDisplayProfile ResolveDisplayProfile()
        {
            ...
            CCS_LocalPlayerOfflineBootstrap legacyBootstrap = GetComponent<CCS_LocalPlayerOfflineBootstrap>();
        ...
        private bool TryWireSceneCameraRig(out CCS_CharacterCameraController sceneCameraRig)
        {
            ...
            CCS_PlayerSessionEvents.RaiseLocalPlayerReady(
                new CCS_PlayerSessionContext(
                    OwnerClientId,
                    gameObject,
        ...
        {
            GameObject sceneCameraRigObject = GameObject.Find(CCS_NetcodeConstants.SceneCameraRigName);
        ...
        {
            if (scene.path != CCS_NetcodeConstants.MasterTestScenePath
                && scene.name != CCS_NetcodeConstants.MasterTestSceneName)
            {
                return;
            }
