...
namespace CCS.Modules.CharacterController.Netcode
{
    public sealed class CCS_MultiplayerHostingMenu : MonoBehaviour
    {
        ...
        public void OnHostAndStartClicked()
        {
            ...
            {
                LogHostingValidation(validationError);
                return;
            }
            ...
            string targetSceneName = CCS_NetcodeConstants.MasterTestSceneName;
        ...
        {
            if (!TryValidateForJoin(out string validationError))
            {
                LogJoinValidation(validationError);
                return;
            }
        ...
        {
            if (!TryValidatePlayerNameForJoin(out string validationError))
            {
                LogJoinValidation(validationError);
                return;
            }
        ...
        private void ApplyDefaultUiValues()
        {
            ...
            {
                manualAddressInput.text = CCS_NetcodeConstants.DefaultLocalhostAddress;
            }

            if (manualPortInput != null && string.IsNullOrWhiteSpace(manualPortInput.text))
            {
                manualPortInput.text = CCS_NetcodeConstants.DefaultServerPort.ToString(CultureInfo.InvariantCulture);
            }
        ...
        private void RebuildServerList()
        {
            ...
            if (emptyServerListText != null)
            {
                ...
                emptyServerListText.text = CCS_NetcodeConstants.EmptyServerListMessage;
            }
        ...
        {
            if (emptyServerListText == null || serverEntries.Count > 0)
            {
                return;
            }
        ...
        private bool HasSelectedRealHost()
        {
            return selectedServerIndex >= 0
                && selectedServerIndex < serverEntries.Count
                && serverEntries.Count > 0;
        }
        ...
        private void LoadMasterTestSceneAsNetworkSession()
        {
            ...
            string sceneName = CCS_NetcodeConstants.MasterTestSceneName;
                    ...
                    || sceneEvent.SceneEventType == SceneEventType.LoadComplete)
                {
                    ...
                    if (pendingClientSceneSync
                        && (sceneEvent.SceneEventType == SceneEventType.SynchronizeComplete
                            || sceneEvent.SceneEventType == SceneEventType.LoadEventCompleted
                            || sceneEvent.SceneEventType == SceneEventType.LoadComplete
                            || SceneManager.GetActiveScene().name == CCS_NetcodeConstants.MasterTestSceneName))
                    {
                        pendingClientSceneSync = false;
        ...
        private void RefreshConnectedPlayersText()
        {
            ...
            connectedPlayersText.text =
                $"Players: {connectedCount.ToString(CultureInfo.InvariantCulture)} / {CCS_NetcodeConstants.DefaultMaxPlayers.ToString(CultureInfo.InvariantCulture)}";
        }
        ...
        private bool TryValidateForJoin(out string errorMessage)
        {
            string rawPlayerName = playerNameInput != null ? playerNameInput.text : string.Empty;
            if (string.IsNullOrWhiteSpace(rawPlayerName))
            {
            ...
            if (!HasSelectedRealHost())
            {
                errorMessage = "No local host selected.";
                ShowJoinSelectionWarning();
                return false;
            }
        ...
        private bool HasValidPlayerName(out string errorMessage)
        {
            ...
            {
                errorMessage = CCS_NetcodeConstants.PlayerNameRequiredForJoinWarningMessage;
                return false;
            }
        ...
        private bool HasValidServerName(out string errorMessage)
        {
            ...
            if (string.IsNullOrWhiteSpace(rawName))
            {
                errorMessage = CCS_NetcodeConstants.ServerNameRequiredWarningMessage;
                return false;
            }
        ...
        private void ShowServerNameWarning()
        {
            ...
            {
                serverNameWarningText.text = CCS_NetcodeConstants.ServerNameRequiredWarningMessage;
        ...
        private void ShowJoinSelectionWarning()
        {
            if (emptyServerListText == null)
            {
                return;
            ...
            emptyServerListText.text = CCS_NetcodeConstants.NoLocalHostSelectedWarningMessage;
            emptyServerListText.gameObject.SetActive(true);
        }

        private static void LogHostingValidation(string reason)
        {
            Debug.Log("[Hosting Validation] " + reason);
        }

        private static void LogJoinValidation(string reason)
        {
            Debug.Log("[Join Validation] " + reason);
        }
        ...
        private void ShowPlayerNameWarning()
        {
            ...
            {
                playerNameWarningText.text = CCS_NetcodeConstants.PlayerNameWarningDisplayMessage;
        ...
        {
            ushort port = CCS_NetcodeConstants.DefaultServerPort;
        ...
        {
            errorMessage = string.Empty;
            port = CCS_NetcodeConstants.DefaultServerPort;

            string rawPort = portInput != null ? portInput.text.Trim() : string.Empty;
            if (string.IsNullOrWhiteSpace(rawPort))
            {
                rawPort = CCS_NetcodeConstants.DefaultServerPort.ToString(CultureInfo.InvariantCulture);
            }
