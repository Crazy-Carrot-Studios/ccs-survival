...
// =============================================================================
// SCRIPT: CCS_LocalPlayerOfflineBootstrapper
...
namespace CCS.Modules.CharacterController.Local {
    [MovedFrom(true, "CCS.Modules.CharacterController.Tests", "CCS.Modules.CharacterController.Tests.Runtime", "CCS_MasterTestPlayerOfflineBootstrapper")]
    [DefaultExecutionOrder(-140)]
    public sealed class CCS_LocalPlayerOfflineBootstrapper : MonoBehaviour
    {
        #region Variables
        ...
        {
            return SceneManager.GetActiveScene().name == CCS_ValidationUiConstants.MasterTestSceneName;
        }
        ...
        private IEnumerator TryBootstrapWhenPlayerReady()
        {
            ...
            {
                if (attemptedBootstrap || CCS_NetworkSessionUtility.IsNetworkSessionActive())
                {
                    yield break;
                }
                ...
                if (playerRoot != null)
                {
                    ...
                    bool configured = CCS_LocalPlayerSessionConfigurator.TryConfigureOfflinePlayer(
                        playerRoot,
                        profile,
                        sceneCamera);
                    LogVerbose(
        ...
        private static GameObject FindSpawnedTestPlayerRoot()
        {
            ...
            for (int i = 0; i < roots.Length; i++)
            {
                ...
                if (root != null && root.name == CCS_PlayerPrefabConstants.NetworkedPlayerInstanceName)
                {
                    return root;
                }
            }
        ...
        private CCS_PlayerDisplayProfile ResolveDisplayProfile()
        {
            ...
            CCS_ValidationSpawnController spawnController = FindFirstObjectByType<CCS_ValidationSpawnController>();
        ...
        {
            if (!CCS_ValidationUiConstants.EnableOfflineBootstrapVerboseLogs)
            {
                return;
            }

            Debug.Log("[Local Player Offline Bootstrapper] " + message);
        }
